...
void __no_inline_not_in_flash_func(joybus_send_bytes)(joybus_port_t *port, uint8_t *bytes, uint len) {
...
void __no_inline_not_in_flash_func(joybus_send_byte)(joybus_port_t *port, uint8_t byte, bool stop) {
    pio_sm_put_blocking(port->pio, port->sm, (byte << 24) | (stop << 23));
}
...
uint __no_inline_not_in_flash_func(joybus_receive_bytes)(
    ...
    for (; bytes_received < len; bytes_received++) {
