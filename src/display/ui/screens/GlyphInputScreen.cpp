...
const unsigned char* menuHintIcon(GlyphProfiles::OutputIcon icon)
{
    switch (icon) {
        case GlyphProfiles::OutputIcon::Home:
            return Home12;
        case GlyphProfiles::OutputIcon::Touchpad:
            return Touchpad12;
...
bool menuHintPressed(uint8_t menuButtonIndex, const GamepadState& state)
{
    switch (menuButtonIndex) {
        ...
        case 3: return (state.buttons & GAMEPAD_MASK_A2) != 0;
...
const unsigned char* activeInputIcon(const GamepadState& state, InputMode mode)
{
    ...
    if (state.buttons & GAMEPAD_MASK_A2) return Bitmap_Touchpad_16;
...
bool iconStillActive(const unsigned char* icon, const GamepadState& state, InputMode mode)
{
    ...
    if (icon == Bitmap_Touchpad_16) return (state.buttons & GAMEPAD_MASK_A2) != 0;
